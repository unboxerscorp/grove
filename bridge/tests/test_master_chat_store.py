...
def _enqueue(store: SQLiteBoardStore, *, msg_ts: str, board: str = "dev10") -> SlackChatQueueItem:
...
def test_upsert_and_get_chat_session(tmp_path: Path) -> None:
    ...
    row = store.upsert_chat_session(board="dev10", conversation_id="conv1", surface="slack")
    assert row.conversation_id == "conv1"
    assert row.surface == "slack"
    assert row.status == "active"

    got = store.get_chat_session(board="dev10", conversation_id="conv1", surface="slack")
    assert got is not None and got.id == row.id

    # Idempotent on the unique (board, conversation, surface) key.
    again = store.upsert_chat_session(board="dev10", conversation_id="conv1", surface="slack")
    assert again.id == row.id

    assert store.get_chat_session(board="dev10", conversation_id="missing", surface="slack") is None
...
def test_claim_due_bounded_drain_claims_each_once(tmp_path: Path) -> None:
    ...
    first = store.claim_due_slack_chat_messages(
        board="dev10", now=now, running_stale_before=stale, limit=2
    )
    assert len(first) == 2
    ...
    second = store.claim_due_slack_chat_messages(
        board="dev10", now=now, running_stale_before=stale, limit=10
    )
    assert len(second) == 1
...
def test_chat_queue_metrics_reports_depth_running_oldest(tmp_path: Path) -> None:
    ...
    m = store.chat_queue_metrics(board="dev10", now=now)
    assert m["depth"] == 2
    assert m["running"] == 0
    ...
    m2 = store.chat_queue_metrics(board="dev10", now=now)
    assert m2["depth"] == 1
    assert m2["running"] == 1
