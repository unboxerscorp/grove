...
- **`--project sample` is overloaded.** It selects the **registry/session**
  (`~/.grove/sample`, `config.session`), defaults the **tmux session**
...
- **Address vs display mismatch.** CLI addresses with `project:node`
  ...
  `node@project` (`org.ts:displayNameForProjectNode` → `lead@sample`). You see
  `lead@sample`, you type `sample:lead`. `:` also reads as the tmux pane separator
  (`sample:2.3`).
...
**deprecated alias** that maps to all of them (so today's `--project sample` and
...
Today the pack emits one `Project: sample` line. Split into:
