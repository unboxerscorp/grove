...
- agy 운영 규약: grove에서는 `agent: antigravity`가 보이는 `agy -i` pane로 동작하고, local operator가 허용한 `--dangerously-skip-permissions`는 권한 프롬프트만 줄일 뿐 `.agents/skills`/보드/위임 규약을 바꾸지 않습니다. 헤드리스 실행은 명시적인 일회성 점검에만 사용합니다.
- 터미널 런타임: `tmux`
