...
- 2026-06-06 02:53 KST 기준 최신 live 운영:
  ...
  - 최신 product-code 커밋은 `cc0ec74 fix: align role preset copy with v2 org model`이다. 이후 docs-only handoff 커밋이 HEAD에 추가될 수 있으므로 `git log --oneline -5`로 현재 HEAD를 확인한다.
...
## 1. 현재 product-code = cc0ec74 (트리 클린)
