...
- 2026-06-06 06:24 KST 기준 최신 live 운영:
  ...
  - Slack은 `dev10:2.0`에서 `/Users/chopin/.grove/dev10/run-slack-loop.sh`로 실행한다. `/api/slack/config/status`는 `socket_connected`, `~/.grove/dev10/slack-runtime.json` heartbeat가 갱신된다. 2026-06-06 06:24 KST 기준 Slack pane은 `dev10:2.0`, runtime pid는 `59036`, heartbeat fresh이다. `a9f8478` Slack control item copy cleanup 반영을 위해 같은 pane을 respawn했으며 socket 재연결을 확인했다.
  ...
  - 최신 product/e2e 정합 커밋은 `5d43e2e fix: align web e2e fixtures with lead nodes`이다. docs-only handoff/vision 정합 커밋이 HEAD에 추가될 수 있으므로 `git log --oneline -5`로 현재 HEAD와 product-code HEAD를 구분한다.
  - 최신 검증: `web npm run verify` RED(`web e2e fixtures must not create legacy project-master defaults`) 확인 후 e2e fixture를 `lead -> worker` 직접 구조로 정렬했고, `web npm run verify`, `web npm run typecheck`, `node e2e/tier1/runner.mjs`(7/7), `pnpm check` green(TS/Vitest 54 files 287 tests, bridge pytest 447 passed), `web npm run check` green이다. `web/verify.mjs` core는 mock source/bundle뿐 아니라 `web/e2e/tier1/fixtures.mjs`와 `web/e2e/live.mjs`가 legacy `project-master` 기본값을 만들면 실패한다. 기존 live-e2e registry에 남은 `project-master` 잔여를 삭제하는 cleanup 라인만 허용한다. Slack approve/abort control command preview/completed/denial copy는 `task id`/`approve task` 대신 `item id`/`approve item`/`abort item`을 쓴다. assistant action preview LLM schema는 `assign_item` + `human-facing item`을 사용하고, legacy `assign_task`/`delegate_task` 응답은 parser compatibility alias로 `assign_item`에 정규화한다. notice prompt도 `task id` 대신 `item id`를 보존 대상으로 설명한다. 2026-06-06 06:22 KST에 consolidated web+Slack restart를 수행해 source/gate 변경을 live process에 반영했다. web `started_at=1780694576`, pid `59048`, health 200, uptime 87s at 06:24; Slack pid `59036`, socket_connected, heartbeat age ~3s. 이전 최신 검증인 `web npm run build && npm run verify && npm run typecheck`, `pnpm vitest run src/cli.test.ts`, `pnpm build`, `grove init --help`도 green이었다. web role preset preview는 더 이상 `위임한 범위`를 노출하지 않고, sub-orchestrator scope를 `lead와 직접 합의한 범위`로 설명한다. `web/verify.mjs` core는 `작업 위임/작업 부여/보드 task/위임한 범위/Delegate task/Assign task/Delegate to this node` copy가 source/bundle에 남으면 실패한다. `grove init --help`는 `scaffold grove.yaml and grove-context.md`로 표시되며 `delegation-protocol doc`을 노출하지 않는다. web mock harness는 더 이상 legacy `project-master`를 assignee/default/master-chat/new-project 기본값으로 노출하지 않으며, 기본 master-org default는 mock root, 새 프로젝트 생성 결과는 API 계약과 같은 `lead`다. `web/verify.mjs` core는 mock source/bundle에 `project-master`가 남으면 실패한다. `grove init`은 새 프로젝트에 `grove-context.md`를 쓰며 `grove-protocol.md`/delegation protocol/read-only reviewer scaffold를 더 이상 생성하지 않는다. `grove new-project` 기본 lead role도 project board 중심 문구 대신 direct node communication + human-facing list items 모델을 주입한다. core verify는 mock `/api/status`에서도 node status `active -> running` alias를 검사한다(`2 실행 중/3 대기`에서 frontend를 active로 바꾼 뒤 `3 실행 중/2 대기`). role preset preview/source는 deprecated board/task copy 대신 `사람용 목록 항목`으로 설명하며 `web/verify.mjs`가 해당 copy 회귀를 막는다. context pack과 master Slack/web chat system prompt도 `Human-facing list items are...` 모델로 설명한다. live Chrome smoke에서 기본 sidebar tabs는 `board/team/terminal/integrations/auth`이고 `connect`는 미노출, join 딥링크(`?join=`)는 `.connect` 패널로 진입하고 URL scrub을 확인했다. NodeStatusBar는 `/api/status` 응답 지연 중 `상태 불러오는 중`만 보이고 false `0 전체`를 노출하지 않으며, 응답 후 `4 실행 중 / 0 대기 / 4 전체`로 전환한다. sidebar `4/4 실시간`, active rows `활성/실행 중`, Team delayed `/api/org` 로딩 중 `노드 불러오는 중`, 완료 후 4 nodes/3 edges/3 groups 확인. `/api/status`는 running=4/idle=0, `/api/projects=[dev10]`, `/api/org` default_assignee=`grove-master`, all 4 nodes `pane_exists:true`, token-only `/api/boards=[dev10 task_count=0]`, explicit `X-Grove-Project: dev10` `/api/boards`는 호환 alias `dev-room`과 `dev10`만 반환, `/api/inbox.total=0`, terminal `/api/ws-ticket?kind=terminal&pane_id=dev10:0.0` 200, `grove org --json` nodes 4개 pane live.
- 현재 노드는 `grove-master`이며 `dev10:0.0`, cwd `/Users/chopin/dev/grove`에서 실행된다.
...
## 1. 현재 product-code = a9f8478 (트리 클린)
