...
- web은 원격 접속을 위해 `0.0.0.0:8765`에 떠 있어야 한다. 현재 LAN URL은 `http://192.168.1.186:8765`이다.
...
- `build_assistant_facts`는 현재 `dev10` registry 기준 `agent_health.node_count=3`을 반환한다. facts는 참고 컨텍스트이며 응답의 유일한 상태 소스가 아니다.
