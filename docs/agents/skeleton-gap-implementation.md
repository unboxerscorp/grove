...
이 섹션이 아래의 과거 `PR-A`~`PR-H` 구현 지시보다 우선한다. 아래 PR 표와 검증 목록은 감사 당시의 work item 기록이며, 현재는 회귀 추적용 체크리스트로 해석한다. 새로 열린 갭은 이 섹션에 명시적으로 `open`으로 표시한다.

| PR                          | 현재 상태     | 구현/검증 근거                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| --------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| A 계층/조직도               | closed for v2 | `grove new-project`가 registry에 실 `lead(parent:"")`를 기록하고 공유 `grove-master` registry를 유지한다. `/api/org`는 `grove-master -> project lead -> nodes` 그래프를 노출하며, real `grove-master`가 있으면 legacy `project-master`를 합성하지 않는다. 대표 가드: `src/commands/new-project.test.ts`의 project lead/shared tmux tests, `bridge/tests/test_web_app.py::test_org_nodes_use_grove_master_as_cross_project_root`, `test_org_does_not_synthesize_project_lead_when_grove_master_is_real`, `test_create_project_invokes_new_project_with_literal_argv`.                                               |
| B 프로젝트 cwd              | closed for v2 | `NodeRuntime.cwd`가 registry에 persist되고, spawn cwd 우선순위는 explicit `--cwd` > project registry cwd다. web node creation은 project workspace 또는 payload cwd를 `grove spawn --cwd`로 넘긴다. 대표 가드: `src/commands/spawn.test.ts`의 `uses the target registry cwd...`, `uses explicit --cwd...`, shared tmux registry test, `bridge/tests/test_web_app.py::test_create_node_payload_cwd_overrides_project_workspace`.                                                                                                                                                                                     |
| C cross-project direct 통신 | closed for v2 | CLI 주소 문법은 `[project:]node`이며 `send`/`ask`/`gather`가 target project registry를 로드하되 caller context를 보존한다. web node send는 `X-Grove-Target-Project`로 타깃 프로젝트를 분리하고 audit은 caller project에 남긴다. 대표 가드: `src/project-address.test.ts`, `src/commands/send.test.ts`, `src/commands/ask.test.ts`, `src/commands/gather.test.ts`, `bridge/tests/test_web_app.py::test_node_send_target_project_header_sends_to_target_and_audits_caller_board`.                                                                                                                                    |
| D 고용/해고 소유권          | closed for v2 | 조직 변경은 사람 소유다. CLI `despawn`은 legacy caller metadata와 무관하게 `--operator-override` 없이는 거부한다. web terminate는 operator role, preview, matching `confirmation_id`, subtree summary, audit을 요구한다. 대표 가드: `src/commands/despawn.ts`, `web/src/api.test.ts` terminate cases, `web/src/components/OrgChart.tsx` terminate confirm UI, `bridge/tests/test_web_app.py::test_terminate_node_*`.                                                                                                                                                                                               |
| E 역할 프리셋               | closed        | canonical catalog는 `src/role-presets.ts`이고 `grove spawn --role-preset`, bridge `role_preset`, web NodeForm preset select/preview가 같은 모델을 쓴다. preset id/version은 runtime에 persist된다. 대표 가드: `src/role-presets.test.ts`, `src/commands/spawn.test.ts` role preset cases, `bridge/tests/test_web_app.py::test_create_node_passes_role_preset_to_spawn_cli`, `web/verify.mjs` role preset guards.                                                                                                                                                                                                   |
| F 조직인지/context pack     | closed for v2 | TS/Python context pack은 caller/project/lead/visible org/target role/tmux pane/cwd와 direct communication + human-facing list item 모델을 prepend한다. facts/context는 노드를 가두는 규칙이 아니라 현재 상태를 잃지 않게 하는 보조 정보다. 대표 가드: `src/context-pack.test.ts`, `bridge/tests/test_context_pack.py`, `bridge/tests/test_web_app.py::test_org_payload_does_not_publish_node_to_node_delegation_protocol`, startup docs `AGENTS.md`/`CLAUDE.md`.                                                                                                                                                   |
| G 사람↔MASTER+SSH/터미널    | closed for v2 | Slack/web chat은 live `grove-master`로 라우팅된다. node connect payload는 headless 환경에서 SSH tmux attach fallback을 제공하고, terminal view는 lead/master pane mirror + operator send path를 검증한다. live e2e는 self-call deadlock 방지를 위해 reentrant `/api/master/chat` POST를 정적 차단한다. 대표 가드: `bridge/tests/test_web_app.py::test_node_connect_info_uses_allowed_host_for_headless_ssh`, `test_terminal_allows_lead_pane_view_and_operator_node_send`, `test_master_chat_node_routed_allows_master_from_visible_project`, `web/e2e/live.mjs`, `web/verify.mjs` live master-chat reentry guard. |
| H 가시 지속 노드 기본화     | closed for v2 | production master chat default는 node-routed `grove-master`이며 direct API client는 explicit dev/test fallback으로만 공개 metadata와 함께 사용된다. default assignee는 실제 지속 노드(`grove-master` 또는 프로젝트 `lead`)이고 legacy `project-master`는 기본 모델에 노출하지 않는다. 대표 가드: `bridge/tests/test_assistant.py::test_default_transport_uses_node_routed_*`, `test_direct_fallback_metadata_discloses_dev_test_status`, `bridge/tests/test_web_app.py::test_org_keeps_grove_master_as_default_when_advisor_exists`, `web/verify.mjs` project-master negative guards.                              |

현재 open skeleton gap: 없음. 새 요구가 생기면 위 표에 `open`으로 추가하고, 아래 과거 감사 문구를 직접 backlog로 되살리지 않는다.
