...
| 생성 직후 빈 조직도 | 신규 프로젝트로 전환 직후             | 새(빈) 프로젝트로 조직도/보드가 재로드                          | `web/verify.mjs` `#N1 project switch re-scope + no residue`                                                                                                          | covered |
...
| 전환 잔존 방지     | 조직도           | 전환 후 이전 프로젝트 노드/카드가 화면에 안 남음 | `web/verify.mjs` `#N1 project switch re-scope + no residue`                                                                                                                         | covered |
| terminal WS 재연결 | 터미널           | 전환 후 노드 선택 시 새 프로젝트 ticket로 연결   | `web/verify.mjs` `wsBindOk`/terminal ticket assertions, `web/e2e/api.mjs` ws-ticket project binding                                                                                 | covered |
...
| WS 재연결+지수 백오프    | 보드/터미널 | 소켓 close 시 백오프(상한), 4401은 중단   | `web/verify.mjs` `n4Ok` close/reconnect/4401 assertions                                                                                                                                                          | covered |
| 터미널 연결상태 전이     | 터미널 패널 | connecting→live→reconnecting→error 표시   | `web/verify.mjs` `#N5 terminal connection-state transitions`                                                                                                                                                     | covered |
